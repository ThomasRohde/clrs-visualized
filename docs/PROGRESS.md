...
> **Next task:** **G3** — `articulation-points` (Prob. 20-2, with bridges) in chapter 20, and
> `difference-constraints` (§22.4) in chapter 22. Both are R4 and neither needs new machinery; the
> second is the only row in the backlog that would have been Tier 1 under this file's own rule. See
...
> **Last completed:** **G2** — `maximum-subarray` (Prob. 4-1), one player running the
> divide-and-conquer and then Problem 4-1(d)'s single pass on the same input, so the two halves of
> the trace tape are the Θ(n lg n)/Θ(n) comparison. Before that, **E8**, the zero baseline for
> `array-bars` that unblocked it — a negative value used to draw as
...
| Ch  | Slug                           | Status | Phase | Renderer | Tier-1 algorithms                                                                     |
| --- | ------------------------------ | ------ | ----- | -------- | ------------------------------------------------------------------------------------- |
| 20  | `elementary-graph-algorithms`  | ✅     | D     | R4       | `bfs` ✅, `dfs` ✅, `topological-sort` ✅, `strongly-connected-components` ✅         |
| 21  | `minimum-spanning-trees`       | ✅     | D     | R4       | `mst-kruskal` ✅, `mst-prim` ✅                                                       |
| 22  | `single-source-shortest-paths` | ✅     | D     | R4       | `bellman-ford` ✅, `dag-shortest-paths` ✅, `dijkstra` ✅                             |
...
| 25  | `bipartite-matching`           | ✅     | D+F   | R4 + R5  | `bipartite-matching` ✅, `gale-shapley` ✅, `hungarian` ✅                            |
