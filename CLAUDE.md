...
It starts a dev server if one isn't already up, stops it after, and exits non-zero on any problem. It walks **every** step of every player across light@1440 / dark@1440 / light@900 and reports: console and page errors, players that never reach `data-ready`, canvases that drew nothing, steps with no narration or without exactly one highlighted code line, panel height changing mid-trace, and any registered algorithm embedded in no chapter.
