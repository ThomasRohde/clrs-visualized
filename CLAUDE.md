...
**Don't push a docs-only change to `master` on its own.** `deploy.yml` runs `ci.yml` as its first job, so every push to the default branch pays the full gate — and seven of those eight minutes are this script stepping 79 players through four theme/width combinations in a real browser, which editing `docs/PROGRESS.md` cannot affect. Commit the doc change locally and let the next change that touches the site carry it; a local `master` that is "ahead by 1" is the intended state. A `paths-ignore` on Deploy was considered and rejected: it would leave those pushes with no gates at all rather than a redundant one.
