import { createRecorder, type AlgorithmModule, type Trace } from '../types.ts';

/**
 * RANDOMIZED-QUICKSORT — CLRS §7.3.
 *
 * One line different from §7.1: before partitioning, a uniformly random
 * element of A[p‥r] is swapped into the pivot slot. That is enough to make
 * the running time depend on the coin flips instead of on the data, so no
 * particular input is bad any more.
 *
 * Deliberately a separate module rather than a flag on `quicksort`: the whole
 * point is to run the two side by side on the same array and watch the tapes
 * diverge, and that needs two players.
 */
export function record(input: number[]): Trace {
  const A: Array<number | null> = [null, ...input];
  const n = input.length;
  const { steps, stats, emit } = createRecorder();
  const PQ = 'RANDOMIZED-QUICKSORT';
  const PR = 'RANDOMIZED-PARTITION';
  const PP = 'PARTITION';

  /** RANDOM(p, r) from the book: a uniform integer in [p, r]. */
  const random = (p: number, r: number) => p + Math.floor(Math.random() * (r - p + 1));

  function partition(p: number, r: number): number {
    const x = A[r] as number;
    emit(PP, 1, A, { range: [p, r], pivot: r }, `x = A[${r}] = ${x} (the pivot).`);
    let i = p - 1;
    emit(PP, 2, A, { range: [p, r], pivot: r, i }, `i = ${p - 1}.`);

    for (let j = p; j <= r - 1; j++) {
      stats.comparisons++;
      emit(
        PP,
        3,
        A,
        { range: [p, r], pivot: r, i, j, compare: [j, r] },
        `for j = ${j} to ${r - 1}`,
      );
      if ((A[j] as number) <= x) {
        i++;
        emit(PP, 5, A, { range: [p, r], pivot: r, i, j }, `A[${j}] = ${A[j]} ≤ x, so i = ${i}.`);
        const tmp = A[i];
        A[i] = A[j];
        A[j] = tmp;
        stats.swaps++;
        emit(
          PP,
          6,
          A,
          { range: [p, r], pivot: r, i, j, swap: [i, j] },
          `Exchange A[${i}] and A[${j}].`,
        );
      } else {
        emit(
          PP,
          4,
          A,
          { range: [p, r], pivot: r, i, j },
          `A[${j}] = ${A[j]} > x, leave it in the tail.`,
        );
      }
    }

    const tmp = A[i + 1];
    A[i + 1] = A[r];
    A[r] = tmp;
    stats.swaps++;
    emit(
      PP,
      7,
      A,
      { range: [p, r], pivot: r, i, swap: [i + 1, r] },
      `Exchange A[${i + 1}] and A[${r}] — pivot settles in place.`,
    );
    emit(PP, 8, A, { range: [p, r], settled: i + 1 }, `return ${i + 1}.`);
    return i + 1;
  }

  function randomizedPartition(p: number, r: number): number {
    const i = random(p, r);
    emit(
      PR,
      1,
      A,
      { range: [p, r], chosen: i, marks: [i] },
      `i = RANDOM(${p}, ${r}) = ${i} — A[${i}] = ${A[i]} will be the pivot.`,
    );
    const tmp = A[i];
    A[i] = A[r];
    A[r] = tmp;
    stats.swaps++;
    emit(
      PR,
      2,
      A,
      { range: [p, r], chosen: i, swap: [i, r] },
      `Exchange A[${i}] with A[${r}] so PARTITION finds it in the usual place.`,
    );
    emit(PR, 3, A, { range: [p, r], pivot: r }, `return PARTITION(A, ${p}, ${r}).`);
    return partition(p, r);
  }

  function quicksort(p: number, r: number): void {
    emit(PQ, 1, A, { range: [p, r] }, `RANDOMIZED-QUICKSORT(A, ${p}, ${r}): is ${p} < ${r}?`);
    if (p < r) {
      const q = randomizedPartition(p, r);
      emit(
        PQ,
        2,
        A,
        { range: [p, r], settled: q },
        `q = RANDOMIZED-PARTITION(A, ${p}, ${r}) = ${q}.`,
      );
      emit(PQ, 3, A, { range: [p, q - 1] }, `Recurse left: A[${p}‥${q - 1}].`);
      quicksort(p, q - 1);
      emit(PQ, 4, A, { range: [q + 1, r] }, `Recurse right: A[${q + 1}‥${r}].`);
      quicksort(q + 1, r);
    } else {
      emit(PQ, 1, A, { range: [p, r], base: true }, `Base case: A[${p}‥${r}] has ≤ 1 element.`);
    }
  }

  quicksort(1, n);
  emit(PQ, 1, A, { range: [1, n], done: true }, `Done — A[1‥${n}] is sorted.`);
  return { steps, finalArray: A.slice(1) as number[] };
}

export const randomizedQuicksort: AlgorithmModule = {
  id: 'randomized-quicksort',
  name: 'Randomized Quicksort',
  visualizer: 'array-bars',
  procOrder: ['RANDOMIZED-QUICKSORT', 'RANDOMIZED-PARTITION', 'PARTITION'],
  procedures: {
    'RANDOMIZED-QUICKSORT': {
      title: 'RANDOMIZED-QUICKSORT(A, p, r)',
      indent: [0, 1, 1, 1],
      lines: [
        'if p < r',
        'q = RANDOMIZED-PARTITION(A, p, r)',
        'RANDOMIZED-QUICKSORT(A, p, q-1)',
        'RANDOMIZED-QUICKSORT(A, q+1, r)',
      ],
    },
    'RANDOMIZED-PARTITION': {
      title: 'RANDOMIZED-PARTITION(A, p, r)',
      indent: [0, 0, 0],
      lines: ['i = RANDOM(p, r)', 'exchange A[i] with A[r]', 'return PARTITION(A, p, r)'],
    },
    PARTITION: {
      title: 'PARTITION(A, p, r)',
      indent: [0, 0, 0, 1, 2, 2, 0, 0],
      lines: [
        'x = A[r]',
        'i = p - 1',
        'for j = p to r - 1',
        'if A[j] ≤ x',
        'i = i + 1',
        'exchange A[i] with A[j]',
        'exchange A[i+1] with A[r]',
        'return i + 1',
      ],
    },
  },
  complexity: {
    best: 'Θ(n log n)',
    average: 'Θ(n log n) expected',
    worst: 'Θ(n²)',
    space: 'O(log n) expected',
    stable: 'No',
    inPlace: 'Yes',
    extra: [['Worst case depends on', 'the coin flips, not the input']],
  },
  record,
};
